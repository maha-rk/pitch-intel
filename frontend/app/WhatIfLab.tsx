...
export default function WhatIfLab({ matches }: { matches: Match[] }) {
  ...
  const load = async (m: Match, removeIndex?: number) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8001/what-if/${m.match_id}?${p}`)
      ...
      <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg)', minHeight: 0 }}>
        {!selected ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 52, letterSpacing: '0.1em', color: 'var(--bg5)' }}>What-If Lab</div>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--t3)' }}>How much did that goal actually matter? · understand a past result, not predict a future one</div>
          </div>
        ) : (
