...
export default function MatchCompanion({ matches }: { matches: Match[] }) {
  ...
  const load = async (m: Match) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8001/match-companion/${m.match_id}?${p}`)
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', height: 'calc(100vh - 150px)', border: '1px solid var(--bd)', borderRadius: 8, overflow: 'hidden' }}>
      ...
      {/* Stage */}
      <div role="region" aria-label="Match audio companion" style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg)', minHeight: 0 }}>
        {!selected ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, textAlign: 'center', padding: 24 }}>
            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 52, letterSpacing: '0.1em', color: 'var(--bg5)' }}>Match Companion</div>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--t3)', maxWidth: 480 }}>Understand the match — without seeing it, hearing the broadcast, or speaking its language</div>
          </div>
        ) : (
              ...
              {result && !result.error && result.beats.length > 0 && (
                <>
                  ...
                  <div aria-live="polite" aria-atomic="true" style={{
                    background: '#0A1D14', border: '1px solid var(--green)', borderRadius: 8, padding: '18px 20px',
                    minHeight: 64, display: 'flex', alignItems: 'center',
                  }}>
                    <span style={{ fontSize: 18, lineHeight: 1.6, color: 'var(--t1)', fontWeight: 600 }}>
                        ...
                        return (
                          <div key={i} onClick={() => { idxRef.current = i; if (ttsSupported) play() }}
                            style={{
                              ...
                              background: active ? '#0A1D14' : 'var(--bg2)',
                              border: `1px solid ${active ? 'var(--green)' : 'var(--bd)'}`,
                            }}>
                            <span style={{ flexShrink: 0, width: 42, fontSize: 11, fontWeight: 800, color: active ? 'var(--green)' : 'var(--t3)', paddingTop: 2 }}>
